...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    return NextResponse.json(courierDB.getSurcharges(Number(id)));
  } catch {
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to add surcharge' }, { status: 500 });
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const entry = courierDB.updateSurcharge(Number(body.entry_id), {
      item_name: body.item_name?.trim(),
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    courierDB.deleteSurcharge(Number(entryId));
